...
-------------------------------------------------
SELECT TO_CHAR(BIRTHDATE, 'YYYY-MM-DD') AS 생일
 FROM STUDENT;
-------------------------------------------------
...
-------------------------------------------------------------------------
SELECT D.DNAME AS 학과명, TRUNC(AVG(P.SAL)) AS 평균급여
 FROM PROFESSOR P, DEPARTMENT D, STUDENT S
WHERE P.PROFNO = D.PROFNO
   AND D.DEPTNO = S.DEPTNO
GROUP BY D.DEPTNO, D.DNAME;
...
1)
------------------------------------------------------------------------------------------
CREATE OR REPLACE PROCEDURE MY_PROC(DNO DEPARTMENT.DEPTNO%TYPE)
IS
