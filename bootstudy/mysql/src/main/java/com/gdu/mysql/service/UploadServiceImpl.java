...
public class UploadServiceImpl implements UploadService {
	...
	public void getUploadList(HttpServletRequest request, Model model) {
		...
		int totalRecord = uploadMapper.selectAllUploadCnt();
		int recordPerPage = 10;
		
		pageUtil.setPageUtil(page, recordPerPage, totalRecord);
		
		Map<String, Object> map = new HashMap<>();
		map.put("begin", pageUtil.getBegin() -1);	// * mysql : mysql은 시작이 0부터기 때문에 -1처리
		map.put("end", pageUtil.getEnd() -1);
	
		model.addAttribute("uploadList", uploadMapper.selectUploadList(map));
		model.addAttribute("paging", pageUtil.getPaging("/upload/list"));	// paging 맵핑값을 전달하면 a링크 경로를 반환해준다
