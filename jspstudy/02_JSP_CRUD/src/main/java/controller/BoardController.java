...
public class BoardController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// => 여기서 +1은 /를 의미하며, 맵핑값에 /를 포함하냐 안하냐가 달라진다
		...
		try {
			if(service != null) {
				af = service.execute(request, response);	// 해석 : execute 메서드를 실행하면 actionforward 객체를 반환
			} // 비즈니스 로직이라면
		} catch (Exception e) {
			e.printStackTrace();
		}
		...
		if(af != null) {	// 해석 : 서비스가 실행되었다면
			...
			if(af.isRedirect()) {
				response.sendRedirect(request.getContextPath() + af.getView());
			} else {
