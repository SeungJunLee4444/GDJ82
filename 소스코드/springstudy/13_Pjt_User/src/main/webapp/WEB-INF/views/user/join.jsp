...
<script>
	...
	function fn_birthyear(){
		let year = new Date().getFullYear();				// 현재 년도를 출력하는 함수
		let strYear = '<option value="">년도</option>';
		for(let y = year - 100; y <= year + 1; y++){		// 100년 전부터 현재까지를 범위로
	...
	<div>
		...
		<form id="frm_join" action="${contextPath}/user/join" method="post">
			...
			<div>
				...
				// # daum api를 이용한 도로명주소 처리 : 아래 코드를 그대로 복붙해 사용하면된다 !
